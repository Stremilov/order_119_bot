...
async def book_place(message: types.Message, state: FSMContext):
    ...
    if user.status == "left":
        await message.answer(
            "Отменять бронирование могут только руководители отделов", reply_markup=main_kb_for_user()
        )
        return
...
async def ask_for_date(message: types.Message, state: FSMContext):
    ...
    await message.answer(text="Вы уверены что хотите отменить бронь?", reply_markup=create_cancel_keyboard())
